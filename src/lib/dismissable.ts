...
export function dismissable(blankSelector: string) {
	return (dialog: HTMLDialogElement) => {
		...
		function isBlank(target: EventTarget | null) {
			return target === dialog || (target instanceof Element && target.matches(blankSelector));
		}
		...
		function up(event: PointerEvent) {
			const dragged = Math.abs(event.clientX - startX) > 3 || Math.abs(event.clientY - startY) > 3;
