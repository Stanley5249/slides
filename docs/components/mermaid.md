...
One render feeds both copies. The preview and the enlarged copy are the same SVG string, so both carry the
same internal marker IDs. Browsers resolve `url(#id)` to the first match and the copies are identical, so
they draw the same.
